...
int potCC[N_POTS] = { 24, // CC 24 (Unassigned)
                      25, // CC 25 (Unassigned)
                      26, // CC 26 (Unassigned)
                      27, // CC 27 (Unassigned)
                      7, // CC 24 (Master Volume)
                      };
...
void loop() {
  ...
  // Reading of the Modulation Wheel (Joystick Y)
  // =========================================================
  ...
  if (modTimer < POT_TIMEOUT) {
    if (modMidiState != modMidiPState) {
      if (modMidiState >= 0) {
        controlChange(POT_CH, 1, modMidiState);
        MidiUSB.flush();
      }
  ...
  if (pitchVar > pitchThreshold) {
    ...
    if (pitchMidiState != pitchMidiPState) {
      pitchBend(POT_CH, pitchMidiState);
      MidiUSB.flush();
